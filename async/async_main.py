...
async def run_tuning_job(config, callback=None):
    ...
    instance_id, bid_price = await vast.search_cheapest_instance(
    ...
    new_instance_id = await vast.create_instance(instance_id, docker_image, env_vars, bid_price, callback=callback)
    ...
    # NOW WAIT FOR ACTUAL TRAINING COMPLETION
    print("\n--- Monitoring Training Progress ---")
    training_completed = await monitor_remote_training(new_instance_id, callback)
...
async def monitor_remote_training(instance_id, callback=None, timeout_minutes=30):
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if "🎉 TRAINING COMPLETED SUCCESSFULLY!" in logs:
                    print("--> Detected successful training completion!")
                    return True
                elif "❌ TRAINING FAILED" in logs:
                    print("--> Detected training failure!")
                    return False
